...
export function ConfirmDialog({ open, title, body, confirmLabel, busy, onConfirm, onCancel }: {
  ...
  return createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-black/50 p-4 backdrop-blur-[2px]" role="alertdialog" aria-modal="true" aria-labelledby="confirm-title" aria-describedby="confirm-description" onKeyDown={(e) => { if (e.key === "Escape" && !busy) onCancel(); }} onMouseDown={(e) => { if (e.target === e.currentTarget && !busy) onCancel(); }}>
