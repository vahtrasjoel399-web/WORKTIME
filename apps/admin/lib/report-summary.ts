import { shiftTotal, type PricingType } from "./pricing.ts";
...
export function summarizeSites(shifts: ReportShift[]) {
  const grouped = new Map<string, { id: string; name: string; address: string | null; hours: number; quantities: Map<string, number>; amount: number; isNet: boolean }>();
  for (const shift of shifts) {
    const key = shift.site_id ?? "unassigned";
    const row = grouped.get(key) ?? { id: key, name: shift.site_name ?? "Objekt määramata", address: shift.site_address ?? null, hours: 0, quantities: new Map(), amount: 0, isNet: shift.is_net !== false };
