-- Client billing is the second financial side of the existing completed work.
-- Worker and client prices are both snapshotted on shifts so later rate changes
-- never rewrite historical payroll or invoice data.

alter table public.sites
  add column if not exists client_name text,
  add column if not exists client_reg_code text,
  add column if not exists client_address text;

create table if not exists public.site_client_rates (
  ...
  constraint site_client_rates_dates_check check (effective_to is null or effective_to >= effective_from),
  constraint site_client_rates_unit_check check (
...
create index if not exists site_client_rates_lookup_idx
  on public.site_client_rates(site_id, pricing_type, label, effective_from desc)
  where is_active;

alter table public.shifts
  add column if not exists client_rate_id uuid references public.site_client_rates(id) on delete set null,
  add column if not exists client_pricing_rate numeric(12,4),
  add column if not exists client_calculated_total numeric(14,2);

alter table public.shifts
  drop constraint if exists shifts_client_pricing_rate_check,
  drop constraint if exists shifts_client_calculated_total_check;
alter table public.shifts
  add constraint shifts_client_pricing_rate_check check (client_pricing_rate is null or client_pricing_rate >= 0),
  add constraint shifts_client_calculated_total_check check (client_calculated_total is null or client_calculated_total >= 0);

alter table public.site_client_rates enable row level security;
create policy site_client_rates_read on public.site_client_rates
...
create policy site_client_rates_admin_write on public.site_client_rates
...
grant select, insert, update, delete on public.site_client_rates to authenticated;

drop trigger if exists trg_site_client_rates_updated_at on public.site_client_rates;
create trigger trg_site_client_rates_updated_at before update on public.site_client_rates
...
begin
  ...
  work_day := (new.started_at at time zone 'UTC')::date;
